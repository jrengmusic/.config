...
local function isValidWindow(w)
	...
	if not w:application() then
		return false
	end
	if not w:screen() then
		return false
	end
	local f = w:frame()
...
local function _isScreenshotApp(app)
	...
	if bid == "com.apple.screencaptureui" then
		return true
	end
...
function obj:_ensureOverlays()
	...
	for _, s in ipairs(screens) do
		...
		if not self._overlays[uuid] then
			...
			if wb.ignoresMouseEvents then
				table.insert(behaviors, wb.ignoresMouseEvents)
			end
			cv:behavior(behaviors)

			-- Do not activate Hammerspoon when the canvas is clicked
			if cv.clickActivating then
			...
			if self._suspended then
				self._overlays[uuid]:hide()
			else
				self._overlays[uuid]:show()
			end
		end
	end
...
function obj:_handleMouseMoved()
	if not self.mouseDim then
		return
	end
	if self._suspended then
		return
	end -- no-op while screenshots
	self:_scheduleRedraw()
end
...
function obj:_startWatchers()
	...
	local function safeSubscribe(ev, cb)
		...
		local ok, err = pcall(function()
			self._wf:subscribe(ev, cb)
		end)
		if not ok then
			self._log.w("window.filter event not supported: " .. tostring(ev) .. " -> " .. tostring(err))
		end
	end
