...
function M.setup()
  ...
  vim.keymap.set('v', '<leader>rg', function()
    ...
    vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes('gv"zy', true, false, true), 'x', false)
    local sel = vim.fn.getreg('z')
    vim.fn.setreg('z', saved, saved_type)
    require('core.cmake-picker').replace(sel)
  end, { desc = 'Project replace selection (cmake)' })
...
function M.setupDap()
  ...
  map('<leader>dx', dap.step_out, 'Step out')
