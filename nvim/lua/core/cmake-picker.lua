...
-- Given the discovered submodule -> module_root map, returns one
-- { module, dir } entry per unique framework sibling dir that actually
-- exists, named as a virtual submodule <prefix>_<siblingName> so it sorts
-- and displays alongside that framework's other modules via the existing
-- <prefix>_ priority/grouping logic. juce has neither (vendored, not
-- produced by these frameworks' own tooling) so it naturally yields nothing
-- here.
local FRAMEWORK_SIBLING_DIRS = { 'cast', 'generated' }

local function collect_framework_siblings(seen_modules)
  local result = {}
  local seen_dirs = {}
      ...
      if submodule:match('^' .. prefix .. '_') then
        ...
        for _, sibling in ipairs(FRAMEWORK_SIBLING_DIRS) do
          ...
          if seen_dirs[dir] == nil and vim.fn.isdirectory(dir) == 1 then
            seen_dirs[dir] = true
            table.insert(result, { module = prefix .. '_' .. sibling, dir = dir })
          end
        end
        break
      end
...
local function generate_symlink_tree()
  ...
  for _, fc in ipairs(collect_framework_siblings(seen_modules)) do
    local sibling_files = scan_dir(fc.dir)
    for _, file in ipairs(sibling_files) do
...
function M.files()
  ...
  if compile_db ~= nil then
    ...
    if data ~= nil then
      ...
      -- Second-and-half pass: scan framework sibling dirs (cast/, generated/ — same as explorer)
      for _, fc in ipairs(collect_framework_siblings(seen_modules)) do
        local sibling_files = scan_dir(fc.dir)
        for _, file in ipairs(sibling_files) do
          if seen[file] == nil then
...
local function get_dirs()
  ...
  -- Framework sibling dirs (jam/cast + jam/generated, ___lib___/cast +
  -- ___lib___/generated, etc.) so grep/replace cover curated/generated
  -- content alongside their modules
  for _, fc in ipairs(collect_framework_siblings(module_roots)) do
